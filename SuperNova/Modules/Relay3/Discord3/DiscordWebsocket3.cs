    ...
    {       
        ...
        public override void OnDisconnected(int reason) {
            if (reason == REASON_INVALID_TOKEN) {
                Logger.Log(LogType.Warning, "Discord3 relay: Invalid bot token provided - unable to connect");
                CanReconnect = false;
            }
            
            Logger.Log(LogType.SystemActivity, "Discord3 relay bot closing: " + reason);
            Close();
        }
        ...
        void DispatchPacket(int opcode, JsonObject obj) {
            if (opcode == OPCODE_DISPATCH) {
                ...
                Logger.Log(LogType.Warning, "Discord3 relay: Resuming failed, trying again in 5 seconds");
                Thread.Sleep(5 * 1000);
                Identify();
            }
        }
