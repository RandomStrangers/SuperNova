...
namespace SuperNova.Modules.Relay3 
{
    public class RelayUser3 { public string ID, Nick; }

    public delegate void OnDirectMessage(RelayBot3 bot, string channel, RelayUser3 user, string message, ref bool cancel);
    ...
    {
        public static void Call(RelayBot3 bot, string channel, RelayUser3 user, string message, ref bool cancel) {
    ...
    public delegate void OnChannelMessage(RelayBot3 bot, string channel, RelayUser3 user, string message, ref bool cancel);
    ...
    public abstract class RelayBot3 
    {
        ...
        public void LoadBannedCommands() {
            BannedCommands = new List<string>() { "IRCBot3", "DiscordBot3", "OpRules", "IRCControllers3", "DiscordControllers3" };
            
            if (!File.Exists("text/Ggobalirccmdblacklist.txt")) {
                File.WriteAllLines("text/lobalirccmdblacklist.txt", new string[] {
        ...
        public void HandleDirectMessage(RelayUser3 user, string channel, string message) {
        ...
        public void HandleChannelMessage(RelayUser3 user, string channel, string message) {
            if (IgnoredUsers.CaselessContains(user.ID)) return;
            message = ParseMessage(message).TrimEnd();
            if (message.Length == 0) return;
        ...
        bool HandleListPlayers(RelayUser3 user, string channel, string cmd, bool opchat) {
            ...
            try {
                RelayPlayer3 p = new RelayPlayer3(channel, user, this);
        ...
        public virtual void MessagePlayers(RelayPlayer3 p) {
            Command.Find("Players").Use(p, "", p.DefaultCmdData);
        }
        
                
        bool HandleCommand(RelayUser3 user, string channel, string message, string[] parts) {
        ...
        bool ExecuteCommand(RelayUser3 user, string channel, string cmdName, string cmdArgs) {
            Command cmd = Command.Find(cmdName);
            Player p = new RelayPlayer3(channel, user, this);
        ...
        public bool CanUseCommand(RelayUser3 user, string cmd, out string error) {
            error = null;
        ...
        public sealed class RelayPlayer3 : Player {
            public readonly string ChannelID;
            public readonly RelayUser3 User;
            public readonly RelayBot3 Bot;
            
            public RelayPlayer3(string channel, RelayUser3 user, RelayBot3 bot) : base(bot.RelayName) {
